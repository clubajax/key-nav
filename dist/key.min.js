...
"function" == typeof define && define.amd ? define([ "@clubajax/on" ], factory) : "object" == typeof module && module.exports ? module.exports = factory(require("@clubajax/on")) : (root.returnExports = factory(), 
root.keys = factory(root.on));
}(this, function(on) {
"use strict";
function keys(listNode, options) {
function unhighlight() {
highlighted && highlighted.removeAttribute("highlighted");
}
function highlight(node) {
if (node = fromArray(node), unhighlight(), !node) {
if (!children[0]) return;
node = children[0];
}
return highlighted = node, highlighted.setAttribute("highlighted", "true"), highlighted;
}
...
function scrollTo() {
console.log("scroll");
const top = highlighted.offsetTop, height = highlighted.offsetHeight, listHeight = listNode.offsetHeight;
top - height < listNode.scrollTop ? listNode.scrollTop = top - height : top + 2 * height > listNode.scrollTop + listHeight && (listNode.scrollTop = top - listHeight + 2 * height);
}
options = options || {};
var multiHandle, observer, searchStringTimer, controller = {
log: !1,
...
destroy: function() {
select(), unhighlight(), this.handles.forEach(function(h) {
h.remove();
}), observer && observer.disconnect();
}
}, tableMode = "table" === listNode.localName, inputMode = options.inputMode, canSelectNone = void 0 === options.canSelectNone || options.canSelectNone, shift = !1, meta = !1, searchString = "", searchStringTime = options.searchTime || 1e3, children = tableMode ? listNode.querySelectorAll("td") : listNode.children, selected = select(getSelected(children, options.noDefault)), highlighted = highlight(fromArray(selected)), nodeType = (highlighted || children[0]).localName;
...
case "ArrowDown":
if (tableMode) {
highlight(getCell(children, highlighted || selected, "down")), on.fire(listNode, "key-highlight", {
value: highlighted
});
break;
}
inputMode && (console.log("input"), highlight(getNode(children, highlighted || selected, "down")), 
on.fire(listNode, "key-highlight", {
value: highlighted
})), scrollTo(), e.preventDefault();

case "ArrowRight":
inputMode || (highlight(getNode(children, highlighted || selected, "down")), on.fire(listNode, "key-highlight", {
value: highlighted
}));
break;
...
case "ArrowUp":
if (tableMode) {
highlight(getCell(children, highlighted || selected, "up")), on.fire(listNode, "key-highlight", {
value: highlighted
}), e.preventDefault();
break;
}
inputMode && (highlight(getNode(children, highlighted || selected, "up")), on.fire(listNode, "key-highlight", {
value: highlighted
})), scrollTo(), e.preventDefault();
...
searchNode && (highlight(select(searchNode)), scrollTo()), clearTimeout(searchStringTimer), 
...
pause: function() {
controller.log && console.log("pause");
},
resume: function() {
controller.log && console.log("resume");
},
remove: function() {
controller.log && console.log("remove");
}
...
childList: !0
}))), scrollTo(), multiHandle = on.makeMultiHandle(controller.handles), Object.keys(multiHandle).forEach(function(key) {
controller[key] = multiHandle[key];
}), controller._resume = controller.resume, controller.resume = function() {
scrollTo(), controller._resume();
}, controller;
}
function isSelected(node) {
return !!node && (node.selected || node.hasAttribute("selected"));
}
...
if (++index > children.length - 1) index = -1; else if (isElligible(children, index)) {
node = children[index];
break;
}
if (norecurse-- < 0) return console.log("recurse"), getFirstElligible(children);
}
return node;
}
function getPrev(children, index) {
for (var norecurse = children.length + 2, node = children[index]; node; ) {
if (--index < 0) index = children.length; else if (isElligible(children, index)) {
node = children[index];
break;
}
if (norecurse-- < 0) return console.log("recurse"), getLastElligible(children);
}
return node;
}
function isVisible(node) {
return "none" !== node.style.display && node.offsetHeight && node.offsetWidth;
}
function getFirstElligible(children) {
for (var i = 0; i < children.length; i++) if (isElligible(children, i)) return children[i];
return null;
}
function getLastElligible(children) {
for (var i = children.length - 1; i >= 0; i--) if (isElligible(children, i)) return children[i];
return null;
}
...
for (var index = 0, i = 0; i < children.length; i++) if (children[i] === highlighted) {
index = i;
break;
}
return "down" === dir ? getNext(children, index) : "up" === dir ? getPrev(children, index) : void 0;
}
...
str = str.toLowerCase();
for (var i = 0; i < children.length; i++) if (0 === children[i].innerHTML.toLowerCase().indexOf(str)) return children[i];
return null;
}
